...
enum R2XMLParser {
  ...
  static func parseListObjectsV2(data: Data) throws -> R2ListObjectsResult {
    ...
    for content in root.elements(forName: "Contents") {
      ...
      let etag =
        content.elements(forName: "ETag").first?.stringValue?
      ...
      let storageClassString =
        content.elements(forName: "StorageClass").first?.stringValue ?? "STANDARD"
  ...
  static func parseCreateMultipartUpload(data: Data) throws -> R2CreateMultipartUploadResult {
    ...
    guard let bucket = root.elements(forName: "Bucket").first?.stringValue,
      ...
      throw R2Error.invalidResponse(
        message: "Missing required fields in CreateMultipartUpload response"
      )
    }
  ...
  static func parseCompleteMultipartUpload(data: Data) throws -> R2CompleteMultipartUploadResult {
    ...
    guard let bucket = root.elements(forName: "Bucket").first?.stringValue,
      let key = root.elements(forName: "Key").first?.stringValue,
      let etag = root.elements(forName: "ETag").first?.stringValue
    else {
      throw R2Error.invalidResponse(
        message: "Missing required fields in CompleteMultipartUpload response"
      )
    }
  ...
  static func parseCopyObject(data: Data) throws -> R2CopyObjectResult {
    ...
    guard let etag = root.elements(forName: "ETag").first?.stringValue,
      let lastModifiedString = root.elements(forName: "LastModified").first?.stringValue
    else {
  ...
  static func parseDeleteObjects(data: Data) throws -> R2DeleteObjectsResult {
    ...
    for deletedElement in root.elements(forName: "Deleted") {
      ...
      let deleteMarker =
        deletedElement.elements(forName: "DeleteMarker").first?.stringValue == "true"

      deleted.append(
        R2DeleteObjectsResult.DeletedObject(
          key: key,
          versionId: versionId,
    ...
    for errorElement in root.elements(forName: "Error") {
      guard let key = errorElement.elements(forName: "Key").first?.stringValue,
        ...
        let message = errorElement.elements(forName: "Message").first?.stringValue
      else { continue }

      errors.append(
        R2DeleteObjectsResult.DeleteError(
          key: key,
          code: code,
          message: message
        )
